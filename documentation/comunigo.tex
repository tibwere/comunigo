...
\end{itemize}
e per simulare condizioni di maggiore stress nel testing è stato incluso, come cconsigliato, un parametro \textit{delay} configurabile che permette di specificare un ritardo nell'invio generato in modo random in un intervallo predefinito.
...
\item ciascun peer sono associati due volumi, uno per permettere che l'attività di logging sia persistente l'altro per accedere agli assets web

\item ogni peer è interconnesso con un'istanza unica di Redis al fine di consegnare e prelevare i messaggi;
...
\begin{itemize}
\item \texttt{[GET] /} permette di accedere al portale di comunicazione effettivo se si autorizzati
...
Le risorse statiche, ovvero i file \texttt{*.html} e \texttt{*.js} poiché non soggetti ad alcun processo di compilazione sono stati inseriti all'interno del filesystem del container come volume \textit{readonly} (\texttt{assets}).

Questo ha come vantaggi:
\begin{itemize}
...
\item le funzionalità \texttt{RPUSH} e \texttt{LRANGE} nativamente permettono di coprire il meccanismo di salvataggio e retrieve dei messaggi
\end{itemize}
...
\item Sincronizzare le goroutines evitanto l'uso esplicito di lock
\end{itemize}
...
V_i[i] = V_i[i] + 1
\end{equation}
\item lo inoltra sulle varie connessioni in precedenza aperte verso i restanti partecipanti al gruppo di multicast
\item lo consegna direttamente poiché certamente rispetta la causalità
\end{itemize}

\item alla \textsl{ricezione di un messaggio} da parte del peer $j$-esimo:
\begin{enumerate}
